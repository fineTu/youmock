  ...
  SimditorMention = (function(superClass) {
    ...
    SimditorMention.prototype._init = function() {
      ...
      if (this.opts.mention.items.length > 0) {
        ...
        this._renderPopover();
      } else if (this.opts.mention.searchable) {
        this._renderSearchablePopver();
      } else {
        this.getItems();
      }
      return this._bind();
    ...
    SimditorMention.prototype.getItems = function() {
      var q;
      q = !this.opts.mention.searchable ? '' : this.searchInputEl.val();
      return $.ajax({
        type: 'get',
        url: this.opts.mention.url + '?q=' + q
      }).done((function(_this) {
        return function(result) {
          _this.items = result;
          if (!_this.opts.mention.searchable) {
            return _this._renderPopover();
          } else {
            return _this._renderItemsOnly();
          }
        };
      })(this));
    };
    ...
    SimditorMention.prototype._renderSearchablePopver = function() {
      var _timer, lastSearch;
      this.popoverEl = $('<div class=\'simditor-mention-popover\'>\n  <div class="mention-search-input">\n    <input name="q" id="miq"/>\n  </div>\n  <div class=\'items\'></div>\n</div>').appendTo(this.editor.el);
      this.searchInputEl = this.popoverEl.find('input[name=q]');
      _timer = null;
      lastSearch = 0;
      return this.searchInputEl.on('keyup', (function(_this) {
        return function(e) {
          var n;
          n = Date.now();
          if (_timer && (n - lastSearch) < 500) {
            return;
          }
          lastSearch = n;
    ...
    SimditorMention.prototype._renderItemsOnly = function() {
    ...
    SimditorMention.prototype.filterItem = function() {
      var q;
      q = this.target.text().toLowerCase().substr(1).replace(/'/g, '');
      return $.ajax({
        type: 'get',
        url: this.opts.mention.url + '?q=' + q
      }).done((function(_this) {
        return function(result) {
          ...
          _this.items = result;
          _this._renderItemsOnly();
          return _this.show(_this.target);
        };
      })(this));
    };
